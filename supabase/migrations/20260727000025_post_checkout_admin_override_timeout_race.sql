-- Allow a deliberate admin approve-without-form decision to supersede only an
-- automatic form-timeout cancellation that has not started provider processing.
-- Once cancellation is processing or confirmed, the override remains blocked.
...
begin
  ...
  -- The scheduled timeout may win the application-row lock immediately before
  -- the administrator confirms an intentional override. Supersede that timeout
  -- only while its Stripe cancellation is still unclaimed.
  if p_decision = 'approve_without_form'
    ...
    if not found or v_timeout_action.status not in ('requested','retry') then
  ...
  if v_timeout_override then
    ...
    where action.id = v_timeout_action.id
      and action.status in ('requested','retry');
