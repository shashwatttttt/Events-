-- Repair promo checkout after the reservation expiry-extension guard made promo_code_id fully immutable.
-- The reservation may attach exactly one matching, already-reserved promo redemption while it is still reserved.
-- All commercial amounts, customer/event identity fields, and subsequent promo changes remain immutable.
...
begin
  ...
  if new.promo_code_id is distinct from old.promo_code_id then
    if not (
      old.promo_code_id is null
      ...
      and exists(
        ...
        where redemption.reservation_id = old.id
          and redemption.promo_code_id = new.promo_code_id
